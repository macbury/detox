module Extract
  ...
  # Based on https://github.com/tootsuite/mastodon/blob/main/lib/paperclip/color_extractor.rb
  class ColorFromImage < Service
    MIN_CONTRAST        = 3.0
    ...
    def call
      ...
      background_palette = palette_from_histogram(convert(':source -alpha set -gravity Center -region 75%x75% -fill None -colorize 100% -alpha transparent +region -format %c -colors :quantity -depth :depth histogram:info:', source: image_path, quantity: 10, depth: depth), 10)
      
      # Determine foreground palette from the whole image
      foreground_palette = palette_from_histogram(convert(':source -format %c -colors :quantity -depth :depth histogram:info:', source: image_path, quantity: 10, depth: depth), 10)
    ...
    def convert(args, opts = {})
      Terrapin::CommandLine.new('convert', args).run(opts)
    end
