    ...
    module RSpec
      ...
      class Schema
        ...
        def execute(query, variables = {})
          ...
          hash = result.to_h
          #raise GraphQLError, hash['errors'] if hash['errors']
          hash
        end
...
RSpec::Matchers.define :be_unauthorized_query do
  match do |result|
    result.dig('errors', 0, 'extensions', 'code') == :unauthorized
  end

  failure_message do |result|
    message = "expected query to be unauthorized, but got:\n"
    message + result.to_yaml.lines.drop(1).join.indent(2)
  end
end
